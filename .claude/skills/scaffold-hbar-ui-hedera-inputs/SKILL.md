...
description: >-
  ...
  Hedera address input, 0.0.n, HashScan, or scaffold-hbar-ui Hedera fields.
---
...
| UI | Hook / notes |
|----|----------------|
| `HederaAddressInput` | Uses `useHederaAddressInput` internally from `@scaffold-hbar-ui/hooks`. You can also call the hook directly for headless logic. |
| `HbarInput` | Uses `useHbarInput` from `@scaffold-hbar-ui/hooks` internally. |
...
- Resolution and validation use mirror / Hedera utilities in `@scaffold-hbar-ui/hooks` (`hederaUtils`, debounced input).

## HbarInput

- Native HBAR entry with USD mode; defaults align with Hedera testnet pricing unless configured otherwise.
- For low-level state without the styled field, use `useHbarInput` from `@scaffold-hbar-ui/hooks`.
