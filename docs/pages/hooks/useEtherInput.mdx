...
# useEtherInput Hook (backward-compatible alias)

The `useEtherInput` hook is a **backward-compatible alias** for [useHbarInput](/hooks/useHbarInput). It defaults `chain` to **mainnet (ETH)**. For Hedera-first apps, use **useHbarInput** (defaults to hederaTestnet / HBAR). Same parameters and return values; only the default chain differs.
